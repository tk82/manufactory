...
check_reslut_GE_25()
{
	...
	while [ 1 ];do
		...
		if [ $file_exit -eq 0 ];then
			...
			line_count=`wc -l < ./GE_25_netperf_net_test.txt`
			for((i = 4; i <= line_count; i+=4))
			do
        			for((j = 2 + i - 4; j < i; j++))
        			do
                			total=`sed -n ''"$j"'p' ./GE_25_netperf_net_test.txt | awk '{print $2}'`
                			let "j+=1"
                			fail_count=`sed -n ''"$j"'p' ./GE_25_netperf_net_test.txt | awk '{print $2}'`
                			if [ $((fail_count * 200)) -le $total ];then
                        			continue
                			else
                        			let "fail_total+=1"
		...
		if [[ $fail_total -eq 0 ]];then
			...
			echo -e "\033[41m [Failed] Some 25 GE Ethernet ports fail the performance test.\t\t\t\t \033[0m"
			break
		fi
	done
}
